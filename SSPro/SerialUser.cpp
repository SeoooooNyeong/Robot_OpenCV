...
void CSSProDlg::send_data_to_dsp()
{
	...
	if (comm_1 && comm_1->m_bIsOpenned)
	{
		...
		if (SendNReturn)
		{
			...
			{ //neck_angle_chg_RL이 true면 ID 23번 모터에 RL_Neck_angle 값 전달
			...
			else if (send_cnt_10ms == 3)
			{
				if (Q_state && MotionCmd != M_NONE)
				{
					...
					case M_BACKWARD:
						...
						break;
					case M_SIDE_LEFT:
						str = CMD_M_SIDE_RIGHT;
						break;
					case M_TURN_LEFT:
						str = CMD_M_TURN_LEFT;
						break;
					case M_TURN_RIGHT:
						str = CMD_M_TURN_RIGHT;
						break;
					case M_TEST:
						str = CMD_M_TEST;
						break;
				...
				else if (MotionCmd == M_FORWARD) {
					static int forward_cnt = 1;
					int forward_max_cnt = 3;
					if (forward_cnt == forward_max_cnt) {
						str = CMD_M_FORWARD;
						forward_cnt = 0;
					}
					else {

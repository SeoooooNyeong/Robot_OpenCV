...
void CSSProDlg::RL_Neck_move() {
	neck_angle_chg_RL = true;
	RL_NeckAngle = 90;
}

void CSSProDlg::bonobo_motion() {
	MotionCmd = M_TEST;
}
...
void CSSProDlg::img_proc()
{
	...
	int label_num = connectedComponentsWithStats(tmp_mask_img, labels, stats, centroids, 8, CV_32S, CCL_DEFAULT);
...
void CSSProDlg::DrawObject() {
	for (int i = 0; i < Color_info_num; i++) {
		...
		for (int j = 0; j < draw_label_num; j++) {
			rectangle(Show_img, Color_info[i].Labels[j].rect, Color_info[i].line_color, 2, 8);
			circle(Show_img, Color_info[i].Labels[j].ct_p, 3, Color_info[i].line_color, -1, 8);
		}
	}
}
